...
remainder: rem"
    ...
    if switch == "start" {
        let Ok((_numone, _numtwo, _operation)) = take_input() else {
            todo!()
        };
...
fn take_input() -> Result<(i32, i32, String), &'static str> {
    ...
    let num_one: i32 = _num1.trim().parse().expect("not a number");
    ...
    io::stdin()
        .read_line(&mut _num2)
        .expect("failed to read the number");
    let num_two: i32 = _num2.trim().parse().expect("not a number");
    ...
    if opparray.contains(&operation.as_str()) {
